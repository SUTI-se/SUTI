...
| 81 | `/xs:schema/xs:complexType[@name='contents']` | `#/$defs/node/properties/contents` | exact | add | done | no schema change; already covered as `node.contents` array of `content` |
| 82 | `/xs:schema/xs:complexType[@name='connection']` | `#/$defs/connection` | exact | add | done | no schema change; already covered via `connection` and `associatedReservation` defs |
| 83 | `/xs:schema/xs:complexType[@name='associatedReservation']` | `#/$defs/associatedReservation` | exact | add | done | no schema change; already covered via `associatedReservation` def and `connection.associatedReservation` reference |
| 84 | `/xs:schema/xs:complexType[@name='contactInfosType']` | `#/$defs/contactInfo` (flattened array usage) | renamed | add | done | no schema change; covered via flattened arrays of `contactInfo` (no standalone wrapper def) |
| 85 | `/xs:schema/xs:complexType[@name='pickupConfirmation']` | *(none)* | missing | add | not-started | deferred for discussion |
| 86 | `/xs:schema/xs:complexType[@name='subOrderType']` | `#/$defs/subOrder` | renamed | rename alias | not-started | deferred for discussion |
| 87 | `/xs:schema/xs:complexType[@name='eventType']` | `#/$defs/event/properties/eventType` | renamed | add | done | no schema change; standalone XSD `eventType` type is unused placeholder, semantics covered by `event.eventType` |
| 88 | `/xs:schema/xs:complexType[@name='eventReport']` | *(none)* | missing | add | not-started | deferred for discussion |
